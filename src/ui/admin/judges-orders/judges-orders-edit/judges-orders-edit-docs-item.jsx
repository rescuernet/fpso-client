...
const DeleteOneDocs = (docsId,docsName) => {
    runInAction(() => {
        AdminJudgesOrdersStore.mediaDel.push(docsName)
        AdminJudgesOrdersStore.judgesOrders.one.docs.splice(docsId,1)
    })
};

const JudgesOrdersDocsItem = (props) => {
    const classes = useStyles();
    const extension = AdminJudgesOrdersStore.judgesOrders.one.docs[props.index].doc.slice(AdminJudgesOrdersStore.judgesOrders.one.docs[props.index].doc.lastIndexOf(".") + 1)
    return (
        <div className={classes.item}>
            <TextField
                ...
                value={AdminJudgesOrdersStore.judgesOrders.one.docs[props.index].title}
                onChange={(e) => {
                    runInAction(() => {
                        AdminJudgesOrdersStore.judgesOrders.one.docs[props.index].title = (e.target.value)
                    })
                }}
                variant="outlined"
                multiline
...
export default observer(JudgesOrdersDocsItem);
